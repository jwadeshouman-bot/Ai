...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', "M7MAD_AI_SECRET_2026_NEVER_CHANGE")

# ====== إعدادات الجلسة (تحل مشكلة الرجوع لصفحة login) ======
app.config.update(
...
.btn-register{background:linear-gradient(135deg,#1a1a1a,#0f0f0f);border:2px solid #ff0040;color:#ff5252;box-shadow:0 4px 15px rgba(255,0,64,0.15);margin-top:12px}
.btn-register:hover{background:linear-gradient(135deg,#ff0040,#d50000);color:#fff;transform:translateY(-2px);box-shadow:0 8px 25px rgba(255,0,64,0.5)}
...
.success{background:rgba(76,175,80,0.15);border:1px solid rgba(76,175,80,0.4);color:#4caf50;padding:12px;border-radius:12px;margin-bottom:20px;text-align:center;font-size:14px}
...
<div class="box">
<div class="logo"><div class="logo-icon">🤖</div><h1>M7MAD_FF AI</h1><p>سجل دخولك أو أنشئ حساب جديد</p></div>
...
<button type="submit" class="btn" onclick="document.getElementById('actionField').value='login'">🚀 دخول</button>
<button type="submit" class="btn btn-register" onclick="document.getElementById('actionField').value='register'">✨ إنشاء حساب جديد</button>
</form>
...
.role-badge{background:rgba(255,215,0,0.25);color:#ffd700;padding:4px 10px;
...
<div class="header">
    ...
    <div class="header-actions">
        ...
        {% if role == 'admin' %}<div class="role-badge">👑 ADMIN</div>{% endif %}
...
def login_page():
    if request.method == 'POST':
        ...
        if not username or not password:
            return render_template_string(LOGIN_HTML, error="أدخل البيانات كاملة", success=None)
        ...
        if action == 'login':
            if username in users and users[username]['password'] == password:
                ...
                return render_template_string(LOGIN_HTML, error="اسم المستخدم أو كلمة المرور غلط", success=None)

        # ===== إنشاء حساب جديد =====
        elif action == 'register':
            if username.lower() == 'admin':
                return render_template_string(LOGIN_HTML, error="هذا الاسم محجوز ❌", success=None)
            if username in users:
                return render_template_string(LOGIN_HTML, error="اسم المستخدم موجود مسبقاً ⚠️", success=None)
            if len(password) < 4:
                return render_template_string(LOGIN_HTML, error="كلمة المرور قصيرة (4 أحرف على الأقل)", success=None)
            if len(username) < 3:
                return render_template_string(LOGIN_HTML, error="اسم المستخدم قصير (3 أحرف على الأقل)", success=None)
